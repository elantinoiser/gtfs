...
#Datos del 16 de julio
jul_16 <- julio %>%  select(id_vehicle, Id, TIMESTAMP, VEHICLE, ROUTEID, STARTTIME, STARTDATE, SCHEDULE_RELATIONSHIP, LABEL, LATITUDE, LONGITUDE, BEARING, ODOMETER, SPEED, CURRENTSTATUS, LECTURA, CST6CDT, as_date, as_hour) %>% filter(as_date == "2020-07-16")
...
jul_16p<- sf::st_as_sf(jul_16, coords = c("LONGITUDE", "LATITUDE"), crs = 4326, agr = "constant")
jul_16_jn <- sf::st_join(jul_16p, gtfs_estatico, join = nngeo::st_nn, maxdist = 100)# Debe correrse library(nngeo) para que trabaje el st_nn
jul_16_jn <-jul_16_jn %>% select(., id_vehicle, geometry, CST6CDT, ruta) %>% filter(ruta=="00L1" |ruta=="00L2")
jul_16_coords<- sf::st_coordinates(jul_16_jn)
jul_16_jn <- cbind(jul_16_jn, jul_16_coords)

#4to paso: función que mida...

jul_fun <- function(i,j) {
  j<-jul_16_jn %>% select(id_vehicle, Y, X, CST6CDT) %>% filter(jul_16_jn$id_vehicle==i) %>%
...
for (i in jul_16_jn$id_vehicle) {
  assign (i, data.frame(jul_fun(i)))
}

#6to paso

unicos <- as.data.frame(unique(jul_16_jn$id_vehicle))
#write.csv(unicos, "C:/Users/85412/Desktop/unicos.csv")


#Séptimo paso. 

lista <- mget(ls(pattern = "2020-07-16-")) # con este comando agrega los dataframes con un patrón a la lista
...
jul_16 <- cbind(unicos, suma)
jul_16 <- cbind(jul_16, rango_t)

jul_16$hrs<- lubridate::as_datetime(jul_16$X2) - lubridate::as_datetime(jul_16$X1)
jul_16$hrs <- jul_16$hrs/60/60

#Octavo paso.

jul_16$hrs <- as.numeric(jul_16$hrs)
jul_16$velocidad <- jul_16$`sapply(lista, function(x) sum(x$dist, na.rm = TRUE))`/jul_16$hrs
names(jul_16) <- c("id_vehicle", "distancia", "t1", "t2", "tiempo", "velocidad")
write.csv(jul_16, "/Users/85412/Desktop/gtfs_rt/julio/jul_16.csv")