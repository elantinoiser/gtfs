...
jul_18 <- julio %>%  select(id_vehicle, Id, TIMESTAMP, VEHICLE, ROUTEID, STARTTIME, STARTDATE, SCHEDULE_RELATIONSHIP, LABEL, LATITUDE, LONGITUDE, BEARING, ODOMETER, SPEED, CURRENTSTATUS, LECTURA, CST6CDT, as_date, as_hour) %>% filter(as_date == "2020-07-18")
...
jul_18p<- sf::st_as_sf(jul_18, coords = c("LONGITUDE", "LATITUDE"), crs = 4326, agr = "constant")
jul_18_jn <- sf::st_join(jul_18p, gtfs_estatico, join = nngeo::st_nn, maxdist = 100)# Debe correrse library(nngeo) para que trabaje el st_nn
jul_18_jn <-jul_18_jn %>% select(., id_vehicle, geometry, CST6CDT, ruta) %>% filter(ruta=="00L1" |ruta=="00L2")
jul_18_coords<- sf::st_coordinates(jul_18_jn)
jul_18_jn <- cbind(jul_18_jn, jul_18_coords)

#4to paso: función que mida...

jul_fun <- function(i,j) {
  j<-jul_18_jn %>% select(id_vehicle, Y, X, CST6CDT) %>% filter(jul_18_jn$id_vehicle==i) %>%
...
for (i in jul_18_jn$id_vehicle) {
  assign (i, data.frame(jul_fun(i)))
}

#6to paso

unicos <- as.data.frame(unique(jul_18_jn$id_vehicle))
#write.csv(unicos, "C:/Users/85412/Desktop/unicos.csv")


#Séptimo paso. 

lista <- mget(ls(pattern = "2020-07-18-")) # con este comando agrega los dataframes con un patrón a la lista
...
jul_18 <- cbind(unicos, suma)
jul_18 <- cbind(jul_18, rango_t)

jul_18$hrs<- lubridate::as_datetime(jul_18$X2) - lubridate::as_datetime(jul_18$X1)
jul_18$hrs <- jul_18$hrs/60/60

#Octavo paso.

jul_18$hrs <- as.numeric(jul_18$hrs)
jul_18$velocidad <- jul_18$`sapply(lista, function(x) sum(x$dist, na.rm = TRUE))`/jul_18$hrs
names(jul_18) <- c("id_vehicle", "distancia", "t1", "t2", "tiempo", "velocidad")
write.csv(jul_18, "/Users/85412/Desktop/gtfs_rt/julio/jul_18.csv")